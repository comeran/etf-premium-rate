...
- 定时任务会在每天北京时间 11:00 和 15:00 自动运行（可在工作流文件中修改）
- 可以在 `Actions` 页面查看运行历史
- **注意：** 私有仓库的 scheduled workflows 需要 GitHub Pro/Team/Enterprise 账户，免费账户仅支持公开仓库
...
编辑 `.github/workflows/etf_premium_rate_schedule.yml` 文件中的 cron 表达式：
...
**注意：** 定时任务配置仅在 GitHub Actions 工作流文件中设置，不需要在 `config.yaml` 中配置。
